...
export class TemplateRenderer {
  ...
  private processVerbConjugations(template: string, context: RenderContext): string {
    ...
    return template.replace(verbPattern, (match, form1, form2, form3, form4) => {
      ...
      if (precedingText.endsWith('I ') || precedingText.endsWith('{subject_subjective} ') && context.subjectPOV === 'FIRST_PERSON') {
        return form1; // First person singular
      } else if (precedingText.endsWith('we ') || precedingText.endsWith('{subject_subjective} ') && context.subjectPOV === 'FIRST_PERSON_PLURAL') {
        return form2 || form1; // First person plural (or fallback)
      } else if (precedingText.endsWith('you ') || 
                 (precedingText.endsWith('{subject_subjective} ') && context.subjectPOV === 'SECOND_PERSON') ||
                 (precedingText.endsWith('{dominant_subjective} ') && context.dominantPOV === 'SECOND_PERSON')) {
